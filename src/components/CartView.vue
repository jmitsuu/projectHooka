...
<template>
  <div v-if="store.modalCart" class="fixed  top-20 w-screen z-50 bg-opacity-90 bg-black h-96 p-2">
      ...
      <div class="py-2 px-4" v-for="item in store.cart" :key="item.title">
        <div class="flex gap-2 items-center">
          ...
          <p class="text-gray-200 font-bold text-[0.8rem]">
            {{ item.title }} - {{ item.price.toFixed(2) }} R$
          </p>
    ...
    <div class="absolute bottom-2 gap-4 flex justify-center w-full">
      ...
      <a
        class="text-[1.3rem] px-2 bg-yellow-600 rounded-md cursor-pointer"
